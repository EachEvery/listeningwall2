<template>
  ...
  >
    <div ref="card" class="h-full bg-white">
      <div class="absolute pin-t pin-x" id="launch-builder" style="height: 7rem;" @click="toggle"></div>
      ...
      <div class="flex flex-col">
        <div class="flex justify-center mb-16 mt-8 px-8">
          <base-button @click="toggle">
            ...
            >{{ __("YOUR POEM") }}</caps>
            <caps v-else class="text-5xl text-center transition uppercase tracking-wide">{{ title }}</caps>
          </base-button>
        ...
        <div class="flex justify-end px-16">
          ...
          >
            <div class="flex flex-col">
              ...
              >
                <div v-html="svg('add-circle', 'w-10 h-10 p-1')"></div>
                <caps class="font-black text-2xs leading-normal mt-1 block">
                  Add Common
                  <br />Words
                </caps>
              </base-button>
              ...
              >
                <div v-html="svg('trash', 'w-10 h-10 p-1')" :class="{ 'text-red': confirmClear }"></div>
                <caps class="font-black text-2xs leading-normal mt-1 block" v-if="!confirmClear">
                  Clear All
                  <br />Words
                </caps>
                <caps class="text-red text-2xs leading-normal mt-1 block" v-if="confirmClear">
                  Tap again
                  <br />to Confirm
                </caps>
              </base-button>
            ...
            <div class="flex flex-col flex-grow justify-end" id="drop-targets">
              <drop-target
                icon="duplicate"
                :dragging="isDragging"
                color="#1f9d55"
...
export default {
  ...
  methods: {
    ...
    clearLines() {
      this.words
        ...
        .forEach(word => {
          let isFromWordBank =
            word.source === undefined || word.source.id === undefined;
    ...
    getWordStyle(word) {
      let fadeWord = this.fadeInWordInfo && this.popoverWord.id !== word.id;
    ...
    initDragDrop() {
      ...
      droppable(".droppable", {
        ...
        accept: function(draggable) {
          ...
          return (
            this.id !== "word-bank" && !(this.id === "my-words" && wordIsHelper)
          );
        }
      });
  ...
  computed: {
    ...
    popoverStyle({ popoverWord }) {
      ...
      let left =
        $word.offset().left + $word.outerWidth() / 2 - $card.offset().left;
